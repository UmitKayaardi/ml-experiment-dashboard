...
// Google Sheet ID — public, readable by anyone with the link.
const SHEET_ID = "1DG-xLRM9wnNccXowsMqlbcxbSAScQAez3FeuH1MN3xw";

// The Google Visualization JSON API works from any browser without authentication.
// credentials: 'omit' is set on every fetch to ensure the browser never sends
// Google session cookies — all users receive the same anonymous, public response
// regardless of which Google account they are signed into in their browser.
...
async function fetchExperiments(): Promise<Experiment[]> {
  // credentials: 'omit' prevents sending any Google browser cookies so that
  // the request is always treated as anonymous — identical for every user.
  const res = await fetch(GVIZ_URL, { credentials: "omit" });
  if (!res.ok) throw new Error(`Failed to fetch data (HTTP ${res.status})`);
  const text = await res.text();

  // gviz wraps the response as JSONP: /*O_o*/\ngoogle.visualization.Query.setResponse({...});
  ...
  return (table.rows ?? []).map((row, idx) => {
    ...
    return {
      id: idx + 2, // sheet row number (row 1 = header, data starts at row 2)
      model: rawModel,
      dataset: rawDataset,
